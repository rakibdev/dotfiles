...
function M.gotoHunk(state, direction)
  ...
  for _, change in ipairs(changes) do
    ...
    if direction == 'next' then
      if start > curLine then foundChange = change; break end
    else
      if start < curLine then foundChange = change end
    end
  end
...
function M.makePatch(state)
  ...
  if #intersecting == 0 then return end
  ...
  table.insert(patch, string.format("@@ -%d,%d +%d,%d @@",
    o.start_line - ctxB, ctxB + oCount + ctxA,
    m.start_line - ctxB, ctxB + mCount + ctxA))

  for l = o.start_line - ctxB, o.start_line - 1      do table.insert(patch, " " .. origLines[l]) end
  for l = o.start_line,        o.end_line - 1         do table.insert(patch, "-" .. origLines[l]) end
  for l = m.start_line,        m.end_line - 1         do table.insert(patch, "+" .. modLines[l])  end
  for l = o.end_line,          o.end_line - 1 + ctxA  do table.insert(patch, " " .. origLines[l]) end
...
function M.stageUnstage(state, action)
  ...
  if not patchText then return end

  local gitArgs = { "apply", "--cached", "--unidiff-zero", "-" }
  if action == 'unstage' then table.insert(gitArgs, 3, "--reverse") end
...
function M.discard(state)
  local sel = state.selected
  if not sel or sel.group == 'staged' then return end

  local patchText = M.makePatch(state)
  if not patchText then return end

  local choice = vim.fn.confirm("Discard selected changes? This cannot be undone!", "&Yes\n&No", 2)
