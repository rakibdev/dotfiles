...
function M.init(state)
  ...
  local function showPlaceholder()
    ...
    for _, l in ipairs(lines) do
      if l:find('%S') then empty = false; break end
    end
    vim.api.nvim_buf_clear_namespace(buf, ns, 0, -1)
    if empty then
      vim.api.nvim_buf_set_extmark(buf, ns, 0, 0, {
        virt_text = { { 'Message (Ctrl+Enter to push)', 'GitPanelPlaceholder' } },
  ...
  local commitAndPush = git.async(function()
    ...
    local frame = 1
    local function tick()
      if not active or not vim.api.nvim_buf_is_valid(buf) then return end
      vim.api.nvim_buf_clear_namespace(buf, ns, 0, -1)
      vim.api.nvim_buf_set_extmark(buf, ns, 0, 0, {
