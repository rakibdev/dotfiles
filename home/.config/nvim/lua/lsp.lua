...
vim.lsp.handlers['textDocument/references'] = function(_, result, ctx)
  if not result or #result == 0 then vim.notify('No references') return end
  ...
  for _, item in ipairs(items) do
    table.insert(lines, string.format('%s:%d  %s', vim.fn.fnamemodify(item.filename, ':~:.'), item.lnum, vim.trim(item.text)))
  end
  ...
  local winnr = vim.api.nvim_open_win(buf, true, {
    ...
    title = ' References (' .. #items .. ') ', title_pos = 'center',
    focusable = true,
  })
...
vim.lsp.config('tsgo', {
  ...
  root_dir  = function(bufnr, on_dir)
    local root = vim.fs.root(bufnr, { 'bun.lock', 'bun.lockb', 'package-lock.json', 'yarn.lock', 'pnpm-lock.yaml', 'package.json', '.git' })
